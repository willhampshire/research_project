...
class Pattern:
    ...
    def __init__(self, period:float, filling:float, size:List[float], lattice:List[List[float]] | None = None,
                 ...
                 form:int=1, pos:List[float]=[0,0], angle:float=0):
        self.period = period
...
def run_simulation(N:int, period:float, thickness:float, filling:float, alpha:float|None=None, beta:float|None=None,
    ...
    MoS2 = Material(f'MoS{phys.sub_2}', dispersive=True,
                    material_path='Materials/MoS2_Munkhbat_o_2022.txt',
                    # 'Materials/WSe2_Zotev.txt' 'Materials/WS2_Munkhbat2022.txt'
                    description=f"TMD layer, MoS{phys.sub_2}.")
    ...
    nan_mask = np.isnan(signalR_)
    signalR_[nan_mask] = 0.5 * (np.roll(signalR_, 1, axis=1) + np.roll(signalR_, -1, axis=1))[nan_mask]

    for ind in [0]:
        if nan_mask[ind]==True:
            signalR_.pop(ind)

    signalR = (signalR_ - np.min(signalR_)) / (np.max(signalR_) - np.min(signalR_))
    ...
    global global_experiment_name
    global_experiment_name = swg.get_summary_name()
...
def main() -> None:
    ...
    gammas = range_in(0., 0.6, 0.1/40)
    asyms = gammas
    asym_type_gamma: bool = True
    ...
    for ax in periods:
        ...
        for t in thicknesses:
            ...
            for ff in filling:
                ...
                for asym in asyms:
                    iterations += 1
                    if asym_type_gamma == True:
                        asym_beta = asym * ax # ax has micron units, so asym does too
                        asyms_new_.append(asym)
                    else:
                        asym_beta = asym
                        asyms_new_.append(asym_beta)
    ...
    global json_dir
    if json_dir:
        json_file = json_dir / 'summary_asym.json'
        with open(json_file, 'w') as file:
            json.dump(periods_p, file, indent=4)

        json_file_meta = json_dir / 'summary_asym_meta.json'
