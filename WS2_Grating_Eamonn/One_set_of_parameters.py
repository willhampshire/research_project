...
Project_name="WS2 grating with unetched part"
...
layer.append([t_TMD2,3])        # Monolayer
layer.append([t_tmd3,2])
layer.append([t_TMD1,3])        # Grating
layer.append([t_WS2_unetched,4]) #000  # unetched WS2 THICKNESS 0
layer.append([t_substrate_2,5]) # Subtrate 2
layer.append([t_substrate_1,6]) # Subtrate 1
# layer.append([t_substrate_0,7]) # Subtrate 0   # Substrate Au/SiO2/Si

layer_sub=[] 
layer_sub.append([0,1])             # Superstrate
layer_sub.append([0,2])             # Monolayer 
layer_sub.append([0,3])             # Spacer layer
layer_sub.append([0,4])             # Grating
layer_sub.append([t_substrate_2,5]) # Subtrate 2
layer_sub.append([t_substrate_1,6]) # Subtrate 1
...
pattern=[]
pattern.append([4,1,1,[0,0],[ax-w,0],0])
...
for i_k in range(0,k_scan.size):
    ...
    print(f"--------------\nDEBUG\n{lbda, material, layer, pattern, u, v, phi, theta, polar, N_ord}")

    df = pd.concat([pd.Series(layer, name='layer'), pd.Series(material, name='material')], axis=1)
    print(df)
    print(pd.Series(pattern))
...
np.savetxt("Preliminary_results\R.txt", R_data)
np.savetxt("Preliminary_results\A.txt", A_data) 
np.savetxt("Preliminary_results\T.txt", T_data)   
np.savetxt("Preliminary_results\R_sub.txt", R_sub_data)
np.savetxt("Preliminary_results\A_sub.txt", A_sub_data) 
np.savetxt("Preliminary_results\T_sub.txt", T_sub_data) 
np.savetxt("Preliminary_results\sample_parameters.txt", sample_parameters)   
# %% Plot result
...
results_name=Project_name+" a="+str("%.3f" % ax)+" tWS2="+str("%.3f" % t_tot)+" etha="+str("%.3f" % etha)+" FF="+str("%.3f" % FF)
title_name=Project_name+"\n"+" a="+str("%.3f" % ax)+" tWS2="+str("%.3f" % t_tot)+" etha="+str("%.3f" % etha)+" FF="+str("%.3f" % FF)
