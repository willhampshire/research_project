...
def fit_simulation(results_dir):
    ...
    for i in [0,1]:
        try:
            # print(i)
            input1 = input(f"Give min max for u then n shape, e.g. 1.4, 1.5: ")
            ...
            input1_lst = input1.rsplit(', ')
            y_min_1 = int(graph_to_fitted(float(input1_lst[0])))
            y_max_1 = int(graph_to_fitted(float(input1_lst[1])))
            ...
            if i==0:
                print("MODE 0")
                try:
                    result = process_fitting(i, y_max_1, y_min_1, -1)
...
json_path = results_top_dir / 'summary_v2.json'
